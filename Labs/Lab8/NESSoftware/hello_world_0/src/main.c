...
void nes_interrupt_handler() {
	...
	//8 is right in game
	//16 is fire in game
	currentButtonStateDebounced = 0;
	if(!(button & (VALUE_TO_SHIFT << NES_CONTROLLER_LEFT))){ //left
		//Sets the bit that makes the tank move to the left
		currentButtonStateDebounced = currentButtonStateDebounced | VALUE_TO_SHIFT << GAME_TANK_LEFT;
	}
	if(!(button & (VALUE_TO_SHIFT << NES_CONTROLLER_RIGHT))){ //right
		//Sets the bit that makes the tank move to the right
		currentButtonStateDebounced = currentButtonStateDebounced | VALUE_TO_SHIFT << GAME_TANK_RIGHT;
	}
	if(!(button & (VALUE_TO_SHIFT))){ //fire
		//Sets the bit that makes the tank fire
		currentButtonStateDebounced = currentButtonStateDebounced | VALUE_TO_SHIFT << GAME_TANK_FIRE;
	}
}
