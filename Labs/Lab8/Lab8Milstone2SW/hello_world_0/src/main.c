...
void sw_interrupt_handler() {
	...
	if(switchState & (1 << SAVE_SCREEN_SHOT_SOFTWARE)){
		...
		for(i = 0; i < SCREEN_BUFFER_NUM_BYTES; i++){ // Iterate through the whole buffer
			framePointer_g_1[i] = framePointer_g[i]; // Save the value of the portion of the buffer
		}
		XTmrCtr_Stop(&myTimer,0); // Stop the timer for the software
		uint32_t value = (uint32_t) XTmrCtr_GetValue(&myTimer,0); // Get the value of the timer
		xil_printf("Software time is %d\r\n", value); // Report the time for software screenshot
	...
	if(switchState & (1 << SAVE_SCREEN_SHOT_HARDWARE)){
	    ...
	    Xil_Out16(XPAR_DMA_0_BASEADDR+DMA_MST_BE_REG_OFFSET, 0xFFFF); //Enables the master register in the DMA
	    Xil_Out8(XPAR_DMA_0_BASEADDR+DMA_MST_GO_PORT_OFFSET, MST_START); //Sends the go command
