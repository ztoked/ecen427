...
-- Date:              Mon Dec 05 15:24:02 2016 (by Create and Import Peripheral Wizard)
...
architecture IMP of user_logic is
  ...
  signal writeAddr                       : std_logic_vector(C_SLV_DWIDTH-1 downto 0);
	signal readAddr                       : std_logic_vector(C_SLV_DWIDTH-1 downto 0);
	signal beatsLeft											: unsigned(31 downto 0);
  ...
  type CMD_CNTL_SM_TYPE is (CMD_IDLE, CMD_READ, CMD_WRITE , CMD_DONE, CMD_WRITE_WAIT_TO_FINISH, CMD_READ_WAIT_TO_FINISH);
        ...
        case mst_cmd_sm_state is
          ...
          when CMD_IDLE =>
            ...
            if ( mst_go = '1' ) then
              ...
              mst_cmd_sm_state <= CMD_READ_WAIT_TO_FINISH;
              ...
              mst_cmd_sm_state       <= CMD_READ;
              mst_cmd_sm_rd_req      <= '1';--High for read, don't need that externally
              mst_cmd_sm_wr_req      <= '0';--High for write , don't need that externally either
              mst_cmd_sm_ip2bus_addr <= readAddr; 
              ...
              mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
					...
					when CMD_WRITE =>
						if ( Bus2IP_Mst_CmdAck = '1' and Bus2IP_Mst_Cmplt = '0' ) then
              mst_cmd_sm_state <= CMD_WRITE_WAIT_TO_FINISH;
            elsif ( Bus2IP_Mst_Cmplt = '1' ) then
              ...
              mst_cmd_sm_state       <= CMD_WRITE;
              mst_cmd_sm_rd_req      <= '0';--High for read, don't need that externally
              mst_cmd_sm_wr_req      <= '1';--High for write , don't need that externally either
              mst_cmd_sm_ip2bus_addr <= writeAddr; 
              mst_cmd_sm_ip2bus_be   <= mst_ip2bus_be(15 downto 16-C_MST_DWIDTH/8 );
              mst_cmd_sm_bus_lock    <= mst_cntl_bus_lock;
            end if;

					when CMD_READ_WAIT_TO_FINISH =>
          ...
          when CMD_DONE =>
						if(beatsLeft = to_unsigned(0,32)) then
							mst_cmd_sm_state    <= CMD_IDLE;
							mst_cmd_sm_set_done <= '1';
							mst_cmd_sm_busy     <= '0';		
							IP2Bus_Interrupt		<= '1';					
						else
							readAddr <= std_logic_vector(unsigned(readAddr) + 4);
							writeaddr <= std_logic_vector(unsigned(writeAddr) + 4);
							mst_cmd_sm_state <= CMD_READ;
