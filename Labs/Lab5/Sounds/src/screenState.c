...
#define UPDATE_BULLET_MAX_COUNT 2//Ticks between bullet updates
...
#define SAUCER_SHOW_UP_FREQUENCY 100 //Seed for how often the saucer showsup
...
static unsigned int * framePointer_g;//To write to the screen
...
static int16_t saucerPos = 0;//X position of saucer
...
int8_t bulletTypes[NUMBER_OF_BULLETS ];//Keeps track of the type of each bullet
...
static int8_t isHittingSaucer(int32_t x, int32_t y) {
	if(saucerMoving && x >= saucerPos && x <= saucerPos + SAUCER_OFFSET  && y >= SAUCER_HEIGHT && y <= SAUCER_HEIGHT + BULLET_SAUCER_HIT_OFFSET ) {//Check X and Y pos of saucer with x and y
		...
		scoreBoard_printNumber(score, (x) * PIXEL_TO_CHUNK , (y - BULLET_SAUCER_HIT_OFFSET ) * PIXEL_TO_CHUNK , RED);//Show the score
...
static void saucer_update() {
	...
	if(saucerMoving == 0) {//If saucer is not moving
		...
		if(random < PIXEL_TO_CHUNK ) {//If it should be
			if(random % PIXEL_TO_CHUNK ) {//If it should be movint right
	...
	if(saucerMoving == SAUCER_MOVING_RIGHT) {
		if(saucerPos >= SCREEN_WIDHT_CHUNK  - SAUCER_WIDTH) {//If it hit the edge
			...
			saucerPos++;//Otherwise keep the saucer moving
		}
	} else {
		if(saucerPos <= 0) {//If saucer hit left edge
			...
			saucerPos--;//Otherwise move the saucer
		}
	}
...
void screenState_update(int32_t currentButtonStateDebounced) {
	...
	if(saucerPrintTime == 1){//If saucer score print timer has expired
		scoreBoard_printNumber(score, (saucer_score_x) * PIXEL_TO_CHUNK , (saucer_score_y - BULLET_SAUCER_HIT_OFFSET ) * PIXEL_TO_CHUNK , BLACK);//Get rid of artifact
