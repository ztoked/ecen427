/*****************************************************************************
* Filename:          E:\427\Lab8\HardWare/drivers/dma_v1_00_a/src/dma_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the dma driver
...
#define DMA_SELFTEST_BUFSIZE  4 /* Size of buffer (n bytes) is fixed to 4 for lite Master */
#define DMA_USER_NUM_REG  4/* Number of registers in slave */

/************************** Variable Definitions ****************************/

static Xuint8 __attribute__((aligned (128))) SrcBuffer[DMA_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[DMA_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus DMA_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < DMA_USER_NUM_REG; write_loop_index++)
    DMA_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < DMA_USER_NUM_REG; read_loop_index++)
    if ( DMA_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (DMA_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
